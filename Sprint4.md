...
- As a user who walks and/or runs, I want to be able to keep track of how many steps I take each day, so I can measure my level of movement.
- As a user who is heart health-conscientious, I want to be able to log my heart rate and blood pressure (systolic and diastolic), so I can more easily store and monitor the behavior of my heart.
- As a user who is heart health-conscientious, I want to be able to see a summarizing representation of my heart health, such as median heart rate and blood pressure, so I can have a better understanding of my heart's usual behavior.


## New Issues to Address

- Add step count tracking feature.
- Add heart health -- heart rate and (systolic, diastolic) blood pressure -- tracking and summarization features.
- Add new UI features that account for these new logs.
- Add backend unit tests for new features.
- Add Cypress frontend unit tests for new UI components.
...
| TestGetHeartHealthSummary_Success | Tests the success case for retrieving the heart health summary. | `backend/handlers/heart_health_test.go` | PASS |
| TestGetExerciseLogs_Empty | Tests the heart health summary retrieval case when heart health records are empty. | `backend/handlers/heart_health_test.go` | PASS |
| TestGetHeartHealthSummary_Unauthorized | Tests the unauthorized failure case for heart health summary retrieval. | `backend/handlers/heart_health_test.go` | PASS |
| TestLogHeartRate_Success | Tests the success case for logging a new heart rate record. | `backend/handlers/heart_health_test.go` | PASS |
| TestLogHeartRate_InvalidRate | Tests the invalid heart rate failure case for new heart rate record logging. | `backend/handlers/heart_health_test.go` | PASS |
| TestLogBloodPressure_Success | Tests the success case for logging a new blood pressure (systolic, diastolic) record. | `backend/handlers/heart_health_test.go` | PASS |
| TestLogBloodPressure_InvalidValues | Tests the invalid blood pressure (systolic, diastolic) failure case for new BP record logging. | `backend/handlers/heart_health_test.go` | PASS |
| TestDeleteHeartHealthEntry_HeartRateSuccess | Tests the success case for deleting a heart rate record from the log. | `backend/handlers/heart_health_test.go` | PASS |
| TestDeleteHeartHealthEntry_BloodPressureSuccess | Tests the success case for deleting a blood pressure (diastolic, systolic) record from the log. | `backend/handlers/heart_health_test.go` | PASS |
...
Allows a user to log a heart rate datapoint.
```
{
    "rate": RATE,         # required, positive integer (BPM)
    "logged_at": TIME     # optional (auto-calculated if omitted)
}
```

### `POST api/heart/blood-pressure`

Allows a user to log a blood pressure datapoint.
```
{
    "systolic":  SYSTOLIC,   # required, positive integer
    "diastolic": DIASTOLIC,  # required, positive integer
    "logged_at": TIME        # optional (auto-calculated if omitted)
}
```

### `GET api/heart/summary`

Allows a user to retrieve a summary of their heart health medians.
```
# NO BODY NECESSARY
```

### `DELETE api/heart/:type/:id`

Allows a user to delete a specific heart health log entry.
```
# NO BODY NECESSARY
# PATH PARAMETERS:
